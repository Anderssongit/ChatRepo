...
nothing and writes nothing.
    ...
    python 2026-09-18/preflight_data.py
    python 2026-09-18/preflight_data.py --excel-dir "D:\\Data\\ExcelData" --json
...
def _last_excel_date(path: Path, sheet, column: str):
    ...
    try:
        ...
        if dates.empty:
            return None, "no parsable Date column"
...
def _price_issue_summary(folder: Path):
    """Classify management_price_issues.csv the way price_repair would."""
    ...
    with path.open(encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            if resolution:                      # written by the patched run
                (repaired if resolution == "rescaled_earlier_segment" else blocked).add(ticker)
                continue
            try:                                # legacy file: classify it now
                ratio = float(row.get("ratio") or "nan")
            except ValueError:
                blocked.add(ticker)
                continue
            (repaired if pr.nearest_scale(ratio) else blocked).add(ticker)
...
def collect(excel_dir: Path, insider_dir: Path, as_of: date):
    ...
    # Upstream inputs this package does not produce.
...
def evaluate(legs, as_of: date):
    ...
    for leg in legs:
        ...
        checks.append(check)
        if not check.ok and check.last_observation is not None:
            blocking.append(check.message)
        if check.last_observation is not None:
...
def main(argv=None) -> int:
    ...
    if issues:
        ...
        print(f"  repairable as unit artifacts: {', '.join(issues['repairable']) or 'none'}")
        print(f"  need manual verification    : {', '.join(issues['blocked']) or 'none'}")
    print("\nUpstream inputs (produced outside this package)")
    for note in notes:
        print(f"  {note}")
    if blocking:
        ...
        print("     (the patched price guard repairs power-of-ten unit artifacts and")
        print("      still blocks anything it cannot prove).")
