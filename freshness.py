...
def month_end_coverage(name: str, last_observation, as_of=None, *,
    ...
    if last > as_of:
        return MonthEndCoverage(name, last, as_of, None, [], max_gap_business_days)
...
def joint_window(coverages: Sequence[MonthEndCoverage]) -> Dict[str, object]:
    ...
    return {
        "last_common_month_end": None if binding is None or missing else binding.last_usable_month_end,
        "binding_component": None if binding is None else binding.name,
        "components_without_coverage": missing,
        "explanation": (
            "Some components cannot substantiate a completed month-end: " + ", ".join(missing)
            if missing else "No component can substantiate a completed month-end."
